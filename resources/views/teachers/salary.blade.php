...
@section('content')
    <div class="container py-4">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('teachers.salaries') }}" method="GET" class="float-end" id="filterForm">
                    <div class="row">
                        <div class="col-md-6">
                ...
                <table class="table">
                    ...
                    <tbody>
                    @php
                        $totalSalary = 0; // Initialize total price variable
                    @endphp
                            ...
                            <td class="text-end">
                                @foreach($courses as $course)
                                    @if($course->id == $section->course_id)
                                        {{ $course->price }}
                                        @php
                                            $totalSalary += $course->price; // Update total price
                                        @endphp
                                    @endif
                                @endforeach
                            </td>
                    ...
                    @endforeach
                        <tr>
                            <th colspan="4"></th>
                            <th class="text-end"> Total: ${{ $totalSalary }}</th>
                        </tr>
                    </tbody>
                </table>
            </div>
